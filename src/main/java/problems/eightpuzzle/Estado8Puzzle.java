...
public class Estado8Puzzle implements Estado, Heuristica {
	...
	private byte heuristica;
	private final byte HEURISTICA_H1 = 1;
	private final byte HEURISTICA_H2 = 2;
	private final byte HEURISTICA_H3 = 3;
	...
	public Estado8Puzzle(byte[][] estado, byte heuristica) {
		this.estado = estado;
		this.heuristica = heuristica;
	}

	public Estado8Puzzle(byte[][] estadoInicial, byte[][] estadoMeta, byte heuristica) {
	...
	private void addSucessores(byte linhaAntiga, byte colunaAntiga, byte novaLinha, byte novaColuna,
			List<Estado> sucessores) {
		byte[][] novo = geraNovaMatriz(linhaAntiga, colunaAntiga, novaLinha, novaColuna);
		sucessores.add(new Estado8Puzzle(novo, estadoMeta, this.heuristica));
	}
	...
	public int h() {
		if (heuristica == HEURISTICA_H2)
			return h2();

		if (heuristica == HEURISTICA_H3)
			return h3();
	...
	//
	// int d = Math.abs(linhaProximo - linhaAnterior) + Math.abs(colunaProximo -
	// colunaAnterior);
	//
